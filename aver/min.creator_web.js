  ...
  for (var i = 0; i < architecture.components.length; i++) {
    for (var j = 0; j < architecture.components[i].elements.length; j++) {
      if (
        ...
        ret.indexComp = i;
        ret.indexElem = j;
        console.log("nombre: ", architecture.components[i].elements[j].name);
        return ret;
        // break;
      }
    }
          ...
          if (is_data){
            ...
            if (matchvalue && !(code_assembly_array[i].includes(".align") || code_assembly_array[i].includes("section") || code_assembly_array[i].includes("data") )){
              ...
              switch(data_to_store.type){
                ...
                case "dword":
                case "integer":
                  if(matchvalue[2].includes(",")) console.log("contengo un vector");
...
function update_vec_popover(pos, register){
  console.log(pos);
  register.actual_pos = pos - 1;
}
...
var uielto_register = {
  ...
  methods: {
    ...
    show_value(register) {
      var ret = 0;
      // if (architecture.components[this._props.component.index].type == 
      //   "v_registers"){
      //     ret = [];
      //   }
...
var uielto_register_popover = {
  ...
  data: function () {
    return { newValue: "", precision: "true", result: [
      { vector_pos: register.name[0]+"[0]", value: "0" },
      { vector_pos: register.name[0]+"[1]", value: "0" },
      { vector_pos: register.name[0]+"[2]", value: "0" },
      { vector_pos: register.name[0]+"[3]", value: "0" },
      { vector_pos: register.name[0]+"[4]", value: "0" },
      { vector_pos: register.name[0]+"[5]", value: "0" },
      { vector_pos: register.name[0]+"[6]", value: "0" },
      { vector_pos: register.name[0]+"[7]", value: "0" },
    ] };
  },
  methods: {
    closePopover() {
      ...
      for (var i = 0; i < architecture.components[3].total_elements; i++){
        switch (view) {
          case "hex":
            ...
            this.result.push({vector_pos : register.name[0] + "["+i.toString()+"]", value : "0x" + register.value.slice(ret_val * length_vext / 4, (ret_val + 1) * length_vext / 4 )});
            break;
          case "bin":
            var ret_val = (512 / length_vext) - i - 1;
            ...
            if (ret.charAt(0) === 1){
              if (length_vext === 8){
                this.result.push([parseInt(ret, 16) - 256 , i]);

              } else if (length_vext === 16){
                this.result.push([parseInt(ret, 16) - 65536 , i]);

              } else if (length_vext === 32){
                this.result.push([parseInt(ret, 16) - 4294967296 , i]);

              } else {
                this.result.push([parseInt(ret, 16) - 18446744073709551616n , i]);
              }
            }
            else
              this.result.push([parseInt(ret, 16) , i]);
            break;
          case "signed":
            var ret_val = (512 / length_vext) - i - 1;
            var ret = register.value.slice(ret_val * length_vext / 4, (ret_val + 1) * length_vext / 4 );
          ...
          case "unsigned":
            ...
            var ret = register.value.slice(ret_val * length_vext / 4, (ret_val + 1) * length_vext / 4 );
            this.result.push([parseInt(ret.toString(10) >>> 0) , i]);
            break;
        }
      }
  ...
  template:
    ...
    '     </b-col>'+
    '     <b-col class="center">'+
    '       <b-table stripped hover :items="result" >' +
    // '         <tbody>'+
    // '           <tr v-for="(value, index) in result" :key=value>'+
    // '             <td> {{register.name}}[{{index}}]</td>'+
    // '             <td>' +

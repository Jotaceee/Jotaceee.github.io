 ...
 */
        ...
        var uielto_register_vec = {
          ...
          methods: {
            ...
            show_value(register) {
              ...
              switch (this.value_representation) {
                case "signed":
                  if (architecture.components[this._props.component.index].type == 
                      ...
                      if (length_vext === 32){
                        ...
                        var ret_val = 7;
                        ret = register.value.slice(ret_val * 64 / 4, (ret_val + 1) * 64 / 4);
                ...
                case "unsigned":
                  if (architecture.components[this._props.component.index].type == 
                      "v_registers"){
                        var ret_val = (512 / length_vext) / 4;
                        ret = register.value.slice(ret_val * length_vext / 4, (ret_val + 1) * length_vext / 4);
                        ret = ret.toString(10) >>> 0;
            ...
            show_value_truncate(register) {
              if (architecture.components[this._props.component.index].type == "v_registers"){
                var ret = this.show_value(register).toString();
                if (ret.length > 8) {
                  ret = ret.slice(0,8) + "...";
                }
